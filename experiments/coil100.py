...
def nolifetime(_algo):
    _algo = {'k_lifetime': 10 * 72}

@ex.named_config
def nospatial(_algo):
    _algo = {'k_spatial': 32 * 32}

@ex.automain
def run(_data, _algo, _rnd, _seed):
    data = COIL100Dataset(config=_data)
    ...
    result = []
    alg.train(data.X)#_train)#, X_val=data.X_val)
    alg.plot_predictions(data.X,#_train[:8], 
                         filepath=results_dir + '/videos_train.png', title='Training Input and Reconstructed Videos')
    ...
    result.append([alg.test(data.X)])#,#_train), #alg.test(data.X_val), 
                   #alg.test(data.X_test)])
    
    for num_labeled in _data['num_labeled']:
        data.generate_labeled_data(num_labeled)
        alg.config['shape1'] = data.X_train_labeled.shape[1]
        alg.build()
        ...
        res.append(alg.test_classifier(data.X_train_labeled, data.Y_train_labeled, stacked=True))
        res.append(alg.test_classifier(data.X_val_labeled, data.Y_val_labeled, stacked=True))
        alg.config['shape1'] = data.X_test.shape[1]
        alg.build()
        alg.stacked_classifier_base.load_weights(results_dir + '/supervised_True.hdf5')
        res.append(alg.test_classifier(data.X_test, data.Y_test, stacked=True))
        result.append(res)
        alg.config['shape1'] = data.X_train_labeled.shape[1]
        alg.build()
        ...
        res.append(alg.test_classifier(data.X_train_labeled, data.Y_train_labeled, stacked=False))
        res.append(alg.test_classifier(data.X_val_labeled, data.Y_val_labeled, stacked=False))
        alg.config['shape1'] = data.X_test.shape[1]
        alg.build()
        alg.endtoend_classifier_base.load_weights(results_dir + '/supervised_False.hdf5')
        res.append(alg.test_classifier(data.X_test, data.Y_test, stacked=False))
        result.append(res)
    return result
