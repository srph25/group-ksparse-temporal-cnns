...
@ex.config
def cfg():
    _data = {'num_labeled': [300, 600, 1000, 2000, 5000, 10000, 54000],
             'rotations': 36,
             'size': 14,
             'zca_epsilon': None,
...
def nolifetime(_algo):
    _algo = {'k_lifetime': 30 * 36}

@ex.named_config
def nospatial(_algo):
    _algo = {'k_spatial': 14 * 14}

@ex.automain
def run(_data, _algo, _rnd, _seed):
    data = MNISTRotatedDataset(config=_data)
